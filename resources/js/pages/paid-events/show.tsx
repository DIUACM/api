...
export default function PaidEventDetailsPage({ paidEvent, registrationInfo }: PaidEventDetailsPageProps) {
    ...
    return (
        <MainLayout>
                        ...
                        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                            <div className="mb-4">
                                <div className="mb-3 flex items-start justify-between gap-3">
                                    <h1 className="flex-1 text-2xl font-bold text-slate-900 sm:text-3xl dark:text-white">{paidEvent.title}</h1>
                            ...
                            {paidEvent.description && (
                                <div className="prose mb-6 max-w-none prose-slate dark:prose-invert">
                            ...
                            {paidEvent.student_id_rules && (
                                <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800/30 dark:bg-blue-900/20">
                                    ...
                                    {paidEvent.student_id_rules_guide && (
                                        <p className="mt-2 text-xs text-blue-700 dark:text-blue-400">Guide: {paidEvent.student_id_rules_guide}</p>
                                    )}
                                </div>
                            )}
                            ...
                            <div className="space-y-4">
                                {paidEvent.departments && paidEvent.departments.length > 0 && (
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {paidEvent.departments.map((dept, index) => (
                                                <Badge key={`dept-${index}`} variant="secondary">
                                                    {dept.name}
                                                </Badge>
                                            ))}
                                        </div>
                                ...
                                {paidEvent.sections && paidEvent.sections.length > 0 && (
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {paidEvent.sections.map((section, index) => (
                                ...
                                {paidEvent.pickup_points && paidEvent.pickup_points.length > 0 && (
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {paidEvent.pickup_points.map((point, index) => (
                                                <Badge key={`pickup-${index}`} variant="outline">
                                                    📍 {point.name}
                                                </Badge>
                                            ))}
                                        </div>
                                ...
                                {paidEvent.lab_teacher_names && paidEvent.lab_teacher_names.length > 0 && (
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {paidEvent.lab_teacher_names.map((teacher, index) => (
                                                <Badge key={`teacher-${index}`} variant="outline">
                                ...
                                <div className="space-y-3 border-t border-slate-200 pt-4 dark:border-slate-700">
                                    ...
                                    {paidEvent.registration_limit && (
                                        <div className="flex items-center justify-between text-sm">
                                            ...
                                            <span className="font-semibold text-slate-900 dark:text-slate-100">{paidEvent.registration_limit}</span>
                                        </div>
                                    )}
                                </div>
                            ...
                            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-600 dark:text-slate-400">{formatDateTime(registrationDeadline)}</p>
                                    </div>
                                </div>
                            </div>
            ...
            <div className="fixed right-0 bottom-0 left-0 z-50 border-t border-slate-200 bg-white/95 p-4 backdrop-blur-sm lg:hidden dark:border-slate-700 dark:bg-slate-900/95">
                <Button
